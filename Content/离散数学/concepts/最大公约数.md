...
> [!abstract] 概述
> ==最大公约数（Greatest Common Divisor, GCD）==是同时整除两个整数 $a$ 和 $b$ 的最大整数，记作 $\gcd(a, b)$。当 $\gcd(a, b) = 1$ 时，称 $a$ 和 $b$ ==互素==（relatively prime）。==最小公倍数（Least Common Multiple, LCM）==是同时被 $a$ 和 $b$ 整除的最小正整数，记作 $\operatorname{lcm}(a, b)$。GCD 和 LCM 满足核心关系 $\gcd(a,b) \cdot \operatorname{lcm}(a,b) = ab$。通过[[算术基本定理]]的素因子分解，GCD 取各素因子指数的==最小值==，LCM 取==最大值==。
